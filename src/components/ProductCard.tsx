  ...
  return (
    <Card className="product-card group overflow-hidden">
      <div className="relative overflow-hidden">
        ...
        <Button variant="ghost" size="icon" className={`absolute top-2 right-2 bg-background/80 hover:bg-background ${wishlisted ? "text-destructive" : ""}`} onClick={handleWishlist}>
          <Heart className={`h-4 w-4 ${wishlisted ? "fill-current" : ""}`} />
        </Button>
        <div className="absolute bottom-2 left-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <Button className="w-full btn-gradient" disabled={!isInStock} onClick={() => addToCart({ id, name, image, price })}>
      ...
      <CardContent className="p-4">
        <div className="flex items-center gap-1 mb-2">
          <div className="flex">{[...Array(5)].map((_, i) => <Star key={i} className={`h-4 w-4 ${i < Math.floor(rating) ? "fill-rating-star text-rating-star" : "text-muted-foreground"}`} />)}</div>
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className="font-bold text-lg text-price">{formatPrice(price)}</span>
          {originalPrice && <span className="price-original">{formatPrice(originalPrice)}</span>}
        </div>
        {isInStock ? isLowStock ? <div className="text-xs text-warning">⚠️ Low stock</div> : <div className="text-xs text-success">✓ In stock</div> : <div className="text-xs text-destructive">❌ Out of stock</div>}
      </CardContent>
    </Card>
  );
};
